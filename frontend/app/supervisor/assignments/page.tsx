    ...
    <div className="flex flex-col h-full gap-3 overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-3 flex-shrink-0">
        <h1 className="text-base sm:text-xl font-bold whitespace-nowrap">
          <span className="hidden sm:inline">{viewMode === 'daily' ? 'Günlük Operasyon Paneli' : 'Haftalık Atama Tablosu'}</span>
          <span className="sm:hidden">{viewMode === 'daily' ? 'Operasyon' : 'Haftalık'}</span>
        </h1>

        <div className="flex rounded-lg border border-gray-200 overflow-hidden text-xs font-semibold">
          <button onClick={() => setViewMode('daily')} className={`px-3 py-1.5 transition-colors ${viewMode === 'daily' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}>Günlük</button>
          <button onClick={() => setViewMode('weekly')} className={`px-3 py-1.5 transition-colors ${viewMode === 'weekly' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}>Haftalık</button>
        </div>
        ...
        {viewMode === 'daily' ? (
          <div className="flex items-center gap-1">
            ...
            <span className="px-2 sm:px-3 py-1.5 text-xs sm:text-sm font-semibold bg-white dark:bg-[#111E38] dark:text-white border border-gray-200 dark:border-[#1E293B] rounded-lg text-center">{dateLabel(date)}</span>
            <button onClick={() => setDate(d => addDays(d, 1))} className="w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100 text-gray-500 text-lg">→</button>
            ...
            <input type="date" value={toISO(date)} onChange={e => e.target.value && setDate(fromISO(e.target.value))} className="hidden md:block ml-1 border border-gray-200 dark:border-[#1E293B] bg-white dark:bg-[#111E38] text-gray-600 dark:text-white rounded px-2 py-1 text-xs" />
          </div>
        ) : (
          <div className="flex items-center gap-1">
            ...
            <span className="px-3 py-1.5 text-sm font-semibold bg-white dark:bg-[#111E38] dark:text-white border border-gray-200 dark:border-[#1E293B] rounded-lg min-w-[210px] text-center">
        ...
        <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto sm:ml-auto">
          {viewMode === 'daily' ? (
            <>
              {dirty && <span className="order-first sm:order-none w-full sm:w-auto text-center sm:text-left text-xs text-amber-600 bg-amber-50 border border-amber-200 px-2 py-1 rounded-full">Kaydedilmemiş</span>}
              ...
              <Button size="sm" isLoading={saving} onClick={handleSave} className="ml-auto sm:ml-0">{dirty ? 'Kaydet ●' : 'Kaydet'}</Button>
            </>
          ) : (
            <>
              ...
              <div className="overflow-x-auto rounded-xl shadow border border-gray-200 bg-white">
                <table className="min-w-full text-sm border-collapse">
                  ...
                  <tbody>
                      ...
                      return (
                        <tr key={emp.id} className={rowBg}>
                          <td className={`sticky left-0 z-10 px-4 py-3 border-r border-[#d1d5db] ${rowBg}`}>
                            ...
                            <div className="text-[10px] text-gray-500 mt-0.5">{emp.role === 'manager' ? 'Yönetici' : emp.role === 'supervisor' ? 'Şef' : 'Personel'}</div>
                          </td>
                          {weekDays.map((d, di) => {
                            const dateStr = toISO(d);
                                  ...
                                  <div className="flex flex-col gap-1.5">
                                    ...
                                    {dayAssignments.length === 0 ? (
                                      ...
                                      dayAssignments.map(a => (
                                        <div key={a.id} className={`rounded-lg border px-2 py-1 ${STATUS_STYLE[a.status] ?? STATUS_STYLE.pending}`}>
                                          <div className="text-[11px] font-semibold leading-tight">{a.task.title}</div>
                                          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                                            {a.task.zone && <span className="text-[9px] opacity-70">📍{a.task.zone.name}</span>}
                                            {a.shift && <span className="text-[9px] opacity-70">⏱{a.shift.name}</span>}
                    ...
                    {employees.length === 0 && (
                      <tr><td colSpan={8} className="text-center py-10 text-gray-400">Aktif personel bulunamadı.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      {viewMode === 'daily' && <div className="flex flex-col md:flex-row gap-4 flex-1 min-h-0">
        ...
        >
          <div className="px-3 py-2.5 border-b border-gray-100 dark:border-[#1E293B] bg-gray-50 dark:bg-[#162543]">
            ...
            <p className="text-[10px] text-gray-400 dark:text-slate-400">{employees.length} aktif</p>
          </div>
            ...
            {employees.map(u => {
              ...
              return (
                ...
                >
                  ...
                  {!isSelected && assigned && <div className="text-[9px] text-indigo-400 dark:text-indigo-400 mt-0.5 pl-5">atanmış ✓</div>}
                </div>
              );
            })}
          </div>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col gap-4 pr-1 pb-4">
          ...
          {selectedForAssign !== null && (
            <div className="sticky top-0 z-10 flex items-center justify-between gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-xl shadow-lg text-sm">
              ...
              <button onClick={() => setSelectedForAssign(null)} className="text-indigo-200 hover:text-white text-lg leading-none">✕</button>
            </div>
          )}
                      ...
                      >
                        ...
                        <div className="flex items-start justify-between gap-1">
                          <button
                            ...
                            onClick={e => { e.stopPropagation(); task.description && setExpandedDescs(prev => ({ ...prev, [task.id]: !prev[task.id] })); }}
                          >
                            {task.title}
                            {task.description && (
