      ...
      <div className="flex flex-wrap items-center gap-3 flex-shrink-0">
        ...
        {viewMode === 'daily' ? (
          <div className="flex items-center gap-1">
            ...
            <span className="px-3 py-1.5 text-sm font-semibold bg-white border border-gray-200 rounded-lg min-w-[180px] text-center">{dateLabel(date)}</span>
            <button onClick={() => setDate(d => addDays(d, 1))} className="w-7 h-7 flex items-center justify-center rounded hover:bg-gray-100 text-gray-500 text-lg">→</button>
            ...
            <input type="date" value={toISO(date)} onChange={e => e.target.value && setDate(fromISO(e.target.value))} className="ml-1 border border-gray-200 rounded px-2 py-1 text-xs text-gray-600" />
          </div>
        ) : (
          <div className="flex items-center gap-1">
        ...
        <div className="flex items-center gap-2 ml-auto">
          {viewMode === 'daily' ? (
            <>
              {dirty && <span className="text-xs text-amber-600 bg-amber-50 border border-amber-200 px-2 py-1 rounded-full">Kaydedilmemiş</span>}
              <Button variant="secondary" size="sm" isLoading={copying} onClick={copyPreviousDay}>↩ Dünü Kopyala</Button>
              <Button variant="secondary" size="sm" onClick={handleExport}>⬇ Excel</Button>
              <Button size="sm" isLoading={saving} onClick={handleSave}>{dirty ? 'Kaydet ●' : 'Kaydet'}</Button>
            </>
          ) : (
            <>
