...
export default function SupervisorStorePage() {
  ...
  useEffect(() => {
    async function load() {
      const [all, dateInfo, zoneList] = await Promise.all([
        assignmentService.getStoreAssignments(),
  ...
  for (const a of assignments) sectionMap[resolveSectionKey(a)].push(a);
      ...
      <div className="flex md:hidden gap-2 mb-6 overflow-x-auto pb-1">
        {SECTIONS.map(s => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap border transition-all duration-150
