...
export default function SupervisorTrainingsPage() {
  ...
  const [form, setForm] = useState({
    title: '', description: '', pdf_url: '', pdf_name: '', visible_to: [] as string[],
  });
  ...
  function canManage(t: Training) {
    return !!user?.unit && t.unit?.id === user.unit.id;
  }
  ...
  function toggleRole(role: string) {
    setForm(f => ({
      ...f,
      visible_to: f.visible_to.includes(role) ? f.visible_to.filter(r => r !== role) : [...f.visible_to, role],
    }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const created = await svc.createTraining({
        title: form.title, description: form.description, pdf_url: form.pdf_url, visible_to: form.visible_to,
      });
    ...
    <div className="min-h-screen bg-[#F9FAFB] dark:bg-[#0A1128] px-4 py-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-400 dark:text-slate-500 mt-0.5">Sizinle ve biriminizle paylaşılan eğitim materyalleri</p>
        </div>
        {user?.unit ? (
          <button
            onClick={openCreate}
          ...
          >
            ...
            Birimime Eğitim Ekle
          </button>
        ) : (
          <p className="text-xs text-amber-600 dark:text-amber-400 max-w-xs">Bir birime atanmadığınız için eğitim oluşturamazsınız.</p>
        )}
      </div>
      ...
      {trainings.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm text-gray-400 dark:text-slate-500">Yöneticiniz veya biriminiz bir eğitim paylaştığında burada görünecek.</p>
        </div>
      ) : (
            ...
            <div key={t.id} className="bg-white dark:bg-[#111E38] rounded-xl shadow-sm border border-gray-100 dark:border-[#1E293B] p-5 flex items-start gap-4">
              ...
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1.5 text-xs px-3.5 py-2 rounded-xl bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition-colors"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="bg-white dark:bg-[#111E38] rounded-2xl shadow-2xl w-full max-w-lg mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 dark:border-[#1E293B]">
              <h2 className="font-bold text-gray-900 dark:text-slate-100">{user?.unit?.name ?? ''} Birimine Yeni Eğitim</h2>
                ...
                >
                  {form.pdf_url ? (
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{uploading ? uploadProgress : 'PDF seçmek için tıklayın'}</p>
